...
def cl_forward(cls,
    ...
    # contrastive loss:
    # \[
    # L_{cl2} = \sum_{i=1}^{N}-log\frac{e^{sim(g_{i},g^{+}_{i})/\tau}}
    # {   \sum_{\substack{j=1 \\ j\neq i}}^{N}e^{sim(g_{i},g_{j})/\tau}
    #     + \alpha*e^{sim(g_{i},g^{-}_{i})/\tau}
    #     + e^{sim(g_{i},g^{+}_{i})/\tau}
    # } \]
    ...
    if num_sent == 3:
        ...
        weights = torch.tensor(
            [[0.0] * z1_z2_cos.size(-1) + [0.0] * z1_z1_cos_removed.size(-1) + [z3_weight] for i in range(z1_z3_cos.size(0))]
        ).to(cls.device)
