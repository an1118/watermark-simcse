...
class Watermark():
    def __init__(self,
                 ...
                 sentiment_tokenizer: Tokenizer = None,
    ...
    # def _sentiment_embedding(self, input_text, model, tokenizer, device):
    ...
    def _watermarking(self, ids, logits, secret_string, measure_threshold, global_sentiment_embedding):
        '''
        ids: Tensor, [[]]
        probs: Tensor, [[]]
        '''
        global_sentiment_embedding = global_sentiment_embedding.to(self.embedding_model.device)
        if len(ids[0]) <= measure_threshold:
            semantic_embedding = self.embedding_model.encode(secret_string, convert_to_tensor=True)
            embedding = (1-self.beta) * semantic_embedding + self.beta * global_sentiment_embedding
            ...
            if measure_entroy >= self.alpha:
                ...
                semantic_embedding = self.embedding_model.encode(measure_text, convert_to_tensor=True)
                embedding = (1-self.beta) * semantic_embedding + self.beta * global_sentiment_embedding
    ...
    def generate_watermarked(self, prompt, original_text, apply_min_new_tokens=False):
        ...
        global_sentiment_embedding = self._sentiment_embedding(original_text, self.sentiment_model, self.sentiment_tokenizer, self.device)
        l2_norm = global_sentiment_embedding.norm(p=2, dim=-1, keepdim=True)  # Compute L2 norm along the last dimension
        global_sentiment_embedding = global_sentiment_embedding / l2_norm  # Normalize
        ...
        for i in range(self.max_new_tokens):
            ...
            logits = self._watermarking(output_ids, logits, self.secret_string, self.measure_threshold, global_sentiment_embedding)   # watermarking
            logits = self._top_k_top_p_filtering(logits, top_k=self.top_k, top_p=self.top_p)   # top-k, top-p filtering
            probs = torch.nn.functional.softmax(logits, dim=-1)   # softmax
            next_id = torch.multinomial(probs, num_samples=1)   # sampling
    ...
    def detection(self, text):
        global_sentiment_embedding = self._sentiment_embedding(text, self.sentiment_model, self.sentiment_tokenizer, self.device)
        global_sentiment_embedding = global_sentiment_embedding.to(self.embedding_model.device)

        watermark_ids = self.watermark_tokenizer.encode(text, return_tensors='pt', add_special_tokens=False).to(self.device)
        
        semantic_embedding = self.embedding_model.encode(self.secret_string, convert_to_tensor=True, device=self.device)
        e = (1 - self.beta) * semantic_embedding + self.beta * global_sentiment_embedding
        ...
        for i in range(len(watermark_ids[0])):
            if i <= self.measure_threshold:
                ...
                measure_entroy = self._next_token_entropy(measure_text, self.measure_model, self.measure_tokenizer, self.device)
                if measure_entroy >= self.alpha:
                    semantic_embedding = self.embedding_model.encode(measure_text, convert_to_tensor=True, device=self.device)
                    e = (1-self.beta) * semantic_embedding + self.beta * global_sentiment_embedding
                    te = self.transform_model(e).tolist()
                    te = [1.0 if x>0.0 else 0.0 for x in te]
                    ve = torch.tensor([te[i] for i in self.mapping_list], device=self.device)
